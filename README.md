...
This section describes how to configure a **second microservice** (e.g., `joputa-service`) to trust the tokens generated by `auth67`.
...
### 5. Protected Controller Example (`JoputaController.java`)
Here is the example of a controller in the **second microservice** that requires the specific `ROLE_JOPUTA`.

```java
@RestController
@RequestMapping("/api/joputa")
public class JoputaController {

    @GetMapping
    @PreAuthorize("hasRole('JOPUTA')")
    public String exclusiveEndpoint() {
        return "Access Granted: You have the ROLE_JOPUTA in your JWT token.";
    }
}
```
